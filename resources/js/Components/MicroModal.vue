    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
      <div class="modal__container w-2/3" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
            ...
            <table class="table-auto w-full text-left whitespace-no-wrap">
                ...
                <tr v-for="customer in customers.value.data" :key="customer.id">
                    <td class="border-b-2 border-gray-200 px-4 py-3">
                      <button type="button" @click="setCustomer({id: customer.id, name: customer.name, kana: customer.kana})" class="text-blue-400">
                        {{ customer.id }}
                      </button>
                    </td>
        ...
        <footer class="modal__footer">
          <button @click="toggleStatus" type="button" class="modal__btn" data-miscromodal-close aria-label="Close this dialog window">閉じる</button>
        </footer>
      </div>
    </div>
  </div>
